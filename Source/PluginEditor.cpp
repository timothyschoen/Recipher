...
: AudioProcessorEditor (&p), audioProcessor (p), keyboard_component(p.keystate, MidiKeyboardComponent::Orientation::horizontalKeyboard)
{
    ...
    shape_slider.onValueChange = [this]() {
        audioProcessor.filter_synth.set_shape(shape_slider.getValue());
    };
    ...
    lpf_q.onValueChange = [this]() {
        audioProcessor.lpf.set_q(lpf_q.getValue());
    };
    ...
    lpf_cutoff.onValueChange = [this]() {
        audioProcessor.lpf.set_cutoff(lpf_cutoff.getValue());
    };
    ...
    sub_slider.onValueChange = [this]() {
        audioProcessor.filter_synth.set_sub(sub_slider.getValue());
    };
    ...
    sample_speed.onValueChange = [this]() {
        audioProcessor.set_sample_speed(sample_speed.getValue());
    };
    addAndMakeVisible(sample_speed);
    
    delay_time.setRange(0, 1.0f);
    delay_time.setValue(1.0f);
    delay_time.onValueChange = [this]() {
        //audioProcessor.filter_synth.set_sub(delay_time.getValue());
    };
    addAndMakeVisible(delay_time);
    
    delay_fb.setRange(0, 1.0f);
    delay_fb.setValue(1.0f);
    delay_fb.onValueChange = [this]() {
        //audioProcessor.filter_synth.set_sub(delay_fb.getValue());
    };
    ...
    lfo_depth.onValueChange = [this]() {
        //audioProcessor.filter_synth.set_sub(lfo_depth.getValue());
    };
    ...
    lfo_rate.onValueChange = [this]() {
        //audioProcessor.filter_synth.set_sub(lfo_rate.getValue());
    };
    ...
    lfo_destination.onValueChange = [this]() {
        //audioProcessor.filter_synth.set_sub(lfo_destination.getValue());
    };
