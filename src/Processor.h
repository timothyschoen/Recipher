...
struct Processor {
...
#else
        auto sample_input = std::vector<float>(44100, 0.0f);
#endif
    ...
    void process (float* input, int num_samples)  {
        ...
        if(realtime_input) {
            ...
            phase_vocoder->process(input_buffer.data(), num_samples);
        }

        std::fill(input, input + num_samples, 0.0f);

        if(lfo_destination == 1) {
            lpf.apply_modulation(lfo_value * 200);
        }
        if(lfo_destination == 2) {
            delay_line.set_delay_samples(lfo_value * 200);
        }
    ...
    void note_on(int note, float velocity, int position) {
        filter_synth.post_midi_message({position, 1, note, velocity});
    }
